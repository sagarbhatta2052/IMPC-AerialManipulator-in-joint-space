...
%%% predictive control for aerial manipulator in the joint space*.
...
q_ref_1 = pi/2*ones(1,End_time/Ts+12);
q_ref_2 = 0*ones(1,End_time/Ts+12);
q_ref_6 = 0*ones(1,End_time/Ts+1);
q_ref_7 = 0*ones(1,End_time/Ts+1);
q_ref_8 = 0*ones(1,End_time/Ts+1);

qd_ref_1 = 0*ones(1,End_time/Ts+12);
qd_ref_2 = 0*ones(1,End_time/Ts+12);
qd_ref_6 = 0*ones(1,End_time/Ts+1);
qd_ref_7 = 0*ones(1,End_time/Ts+1);
qd_ref_8 = 0*ones(1,End_time/Ts+1);

qdd_ref_1 = 0*ones(1,End_time/Ts+12);
qdd_ref_2 = 0*ones(1,End_time/Ts+12);
...
q_ref_3(1,k) = 2*sin(0.2*t);
qd_ref_3(1,k) = 2*0.2*cos(0.2*t);
qdd_ref_3(1,k) = -2*0.2*0.2*sin(0.2*t);

q_ref_4(1,k) = 2*cos(0.2*t);
qd_ref_4(1,k) = -2*0.2*sin(0.2*t);
qdd_ref_4(1,k) = -2*0.2*0.2*cos(0.2*t);

q_ref_5(1,k) = 0.5*2*(0.2*t);
qd_ref_5(1,k) = 0.5*2*0.2;
qdd_ref_5(1,k) = 0;
...
%% Inputs and  References for IMPC control
            ...
            X_ref_8 = zeros(40,1);
                ...
                for i = 0:N1-1
                        ...
                        q_ref_3_1 = 2*sin(0.2*((k+i+1)*Ts));
                        qd_ref_3_1 = 2*0.2*cos(0.2*((k+i+1)*Ts));
                        q_ref_3_2 = 2*sin(0.2*((k+i)*Ts));
                        qd_ref_3_2 = 2*0.2*cos(0.2*((k+i)*Ts));

                        q_ref_4_1 = 2*cos(0.2*((k+i+1)*Ts));
                        qd_ref_4_1 = -2*0.2*sin(0.2*((k+i+1)*Ts));
                        q_ref_4_2 = 2*cos(0.2*((k+i)*Ts));
                        qd_ref_4_2 = -2*0.2*sin(0.2*((k+i)*Ts));

                        q_ref_5_1 = 0.5*2*(0.2*((k+i+1)*Ts));
                        qd_ref_5_1 = 0.5*2*0.2;
                        q_ref_5_2 = 0.5*2*(0.2*((k+i)*Ts));
                        qd_ref_5_2 = 0.5*2*0.2;
                        ...
                        %inputs for next step optimization 
...
E_ref_3 = q_ref_3(1,1:5001) + L2.*cos(q_ref_1(1,1:5001)).*sin(q_ref_2(1,1:5001));
E_ref_4 = q_ref_4(1,1:5001) + (L1 + L2.*cos(q_ref_2(1,1:5001))).*cos(q_ref_1(1,1:5001));
E_ref_5 = q_ref_5(1,1:5001) - L0 -(L1 + L2.*cos(q_ref_2(1,1:5001))).*sin(q_ref_1(1,1:5001));
...
plot( 0:Ts:End_time , q_t(3,:) - 2*sin(0.2*(time_serial)) );
title('X axis error')
xlabel('time(s)')
ylabel('amplitude(m)')
...
plot( 0:Ts:End_time , q_t(4,:) - 2*cos(0.2*(time_serial)) );
title('Y axis error')
xlabel('time(s)')
ylabel('amplitude(m)')
...
subplot(3,1,3)
%plot thita1 error over time horizon
plot( 0:Ts:End_time , q_t(5,:) - 0.5*2*(0.2*(time_serial)));
title('Z axis error')
xlabel('time(s)')
ylabel('amplitude(m)')
set(gcf,'color','w');
...
plot( 0:Ts:End_time-0.01, End_Effect_xx(1,2:5001) - E_ref_3(1,2:5001) );
title('Xe axis error')
xlabel('time(s)')
ylabel('amplitude(m)')
...
plot( 0:Ts:End_time-0.01, End_Effect_yy(1,2:5001) - E_ref_4(1,2:5001) );
title('Ye axis error')
xlabel('time(s)')
ylabel('amplitude(m)')
...
plot( 0:Ts:End_time-0.01, End_Effect_zz(1,2:5001) - E_ref_5(1,2:5001) );
title('Ze axis error')
xlabel('time(s)')
ylabel('amplitude(m)')
...
% L1 = 0.1;
% L2 = 0.2;
% End_Effect_xx = q_t(3,2:5001) + L2.*cos(q_t(1,2:5001)).*sin(q_t(2,2:5001));
% End_Effect_yy = q_t(4,2:5001) + (L1 + L2.*cos(q_t(2,2:5001))).*cos(q_t(1,2:5001));
% End_Effect_zz = q_t(5,2:5001) - L0 -(L1 + L2.*cos(q_t(2,2:5001))).*sin(q_t(1,2:5001));
% 
% E_ref_3 = q_ref_3(1,2:5001) + L2.*cos(q_ref_1(1,2:5001)).*sin(q_ref_2(1,2:5001));
% E_ref_4 = q_ref_4(1,2:5001) + (L1 + L2.*cos(q_ref_2(1,2:5001))).*cos(q_ref_1(1,2:5001));
% E_ref_5 = q_ref_5(1,2:5001) - L0 -(L1 + L2.*cos(q_ref_2(1,2:5001))).*sin(q_ref_1(1,2:5001));
% 
% 
% %% reference tracking plot
% figure
% plot3(q_t(3,2:5001),q_t(4,2:5001),q_t(5,2:5001));
% hold on
% plot3(q_ref_3(1,2:5001),q_ref_4(1,2:5001),q_ref_5(1,2:5001),'--');
% hold on 
% plot3( E_ref_3(1,:),E_ref_4(1,:),E_ref_5(1,:),'-.')
% hold on
% plot3( End_Effect_xx,End_Effect_yy,End_Effect_zz,':')
